    ...
    for iter_idx in range(cfg.promote_max_iters):
        ...
        if cfg.promote_decompose_multi_concept and multi_concept_u:
            ...
            if kept_atoms:
                ...
                # v8.15: pass exclusions through so the annotator's "NOT X"
                # clauses (added by the v8.14 decomposer) actually reach
                # the prompt during mini-annotation. Without this the
                # boundary the decomposer worked out is invisible to the
                # annotator and atom mini-labels regress toward ambiguity.
                atom_as_feats = []
                for a in kept_atoms:
                    feat = {
                        "id": a["atom_id"],
                    ...
                    if a.get("exclusions"):
                        feat["exclusions"] = a["exclusions"]
                    atom_as_feats.append(feat)
                n_seqs = min(
